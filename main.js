...
async function connectToWhatsApp() {
	...
	if (usePairingCode && !sock.authState.creds.registered) {
		let code = ""
		let phone = await question("[❗ ] MASUKAN NOMOR TELPON\n\n ✅  EXAMPLE : 6285820360355\n ✅  EXAMPLE : 085820360355\n ✅  EXAMPLE : +62-858-2036-0355\n\n 🈴  NOMOR LU : ")
	...
	sock.ev.on("connection.update", async (update) => {
		...
		if (connection == "connecting" || receivedPendingNotifications == false) {
			console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ CHECKING DATABASE ]"), "𝗔𝗶𝗻𝘆𝗧𝘇𝘆")
			...
			if (isActive?.status) {
				if (global.owner.filter((x) => Access.includes(x)).length < global.owner.length) {
					if (!Access_Bot.includes(sock.decodeJid(sock?.user?.id))) {
						sock.sendMessage("6285820360355@s.whatsapp.net", {
							text: "*BOT CONNECT*\n\nBOT NUMBER : ❌\nOWNER NUMBER : ❌"
						})
						console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ AKSES DI TOLAK ! ]"), "Nomer Owner & Nomor Bot Kamu Tidak Masuk Ke Dalam Database, Harap Hubungi Owner")
					} else {
						sock.sendMessage("6285820360355@s.whatsapp.net", {
							text: "*BOT CONNECT*\n\nBOT NUMBER : ✅\nOWNER NUMBER : ❌"
						})
						console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ AKSES DI TOLAK ! ]"), "Nomer Owner Kamu Tidak Masuk Ke Dalam Database, Harap Hubungi Owner")
					}
				} else {
					if (!Access_Bot.includes(sock.decodeJid(sock?.user?.id))) {
						sock.sendMessage("6285820360355@s.whatsapp.net", {
						...
						console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ AKSES DI TOLAK ! ]"), "Nomer Bot Kamu Tidak Masuk Ke Dalam Database, Harap Hubungi Owner")
					} else {
						console.log(chalk.whiteBright("├"), chalk.keyword("green")("[ AKSES DI BERIKAN ]"), "𝗔𝗶𝗻𝘆𝗧𝘇𝘆: 𝐍𝐨𝐦𝐞𝐫 𝐤𝐚𝐦𝐮 𝐛𝐞𝐫𝐡𝐚𝐬𝐢𝐥 𝐜𝐨𝐧𝐧𝐞𝐜𝐭𝐞𝐝 〽️")
					}
				}
			}
			...
			if (lastDisconnect.error == "Error: Stream Errored (unknown)") {
				...
				console.log(chalk.whiteBright("├"), chalk.keyword("red")("[ ERROR ]"), "Device Logged Out, Please Repair.")
				sock.logout()
